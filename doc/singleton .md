...
特点：类在调用时才实例化对象，需要添加synchronized使线程安全

缺点：效率很低，每个线程不管是否创建对象，都得等待进入方法。
...
指令重排序：在不影响结果情况下，jvm会优化执行顺序（时间片问题）。在单线程没有问题，在多线程会有问题。

例如：student = new Student();  正常的执行顺序是  先初始化对象，再指向内存空间。但2，3可能会重排序，颠倒顺序。在多线程时，如果顺序颠倒，线程的访问就可能判断有对象，又获取不到对象。

![](../imgs/s01.png)

要解决这个问题需要添加``volatie``修饰，``volatie``修饰会禁止重排序；在初始化对象之后，马上将值写入主存。

## 登记式单例

使用内部类初始化对象，是饿汉模式的改良版，只有在调用的时候才加载；静态内部类加载的时候就初始化，重排序不会被其他线程看到（加了锁）
